package database

import (
	"database/sql"
	"fmt"
	"slander/internal/systems"
	"strings"
	"time"
)

func PostExists(id systems.PostID) (exists bool, err error) {
	row := DB.QueryRow("select exists(select 1 from posts where id = ?)", id)
	err = row.Scan(&exists)
	return
}

func GetPost(id systems.PostID) (systems.Post, error) {
	row := DB.QueryRow("select post_type from posts where id = ?", id)
	var ptstr string
	if err := row.Scan(&ptstr); err != nil {
		return systems.Post{}, err
	}
	post_type := systems.PostTypeFromString(ptstr)
	var (
		parent        int32
		owner         string
		original_body string
		body          string
		timedate      string
	)
	if post_type != systems.Original {
		row = DB.QueryRow("select parent, owner, original_body, body, time from posts where id = ?", id)
		if err := row.Scan(&parent, &owner, &original_body, &body, &timedate); err != nil {
			return systems.Post{}, err
		}
	} else {
		row = DB.QueryRow("select owner, original_body, body, time from posts where id = ?", id)
		if err := row.Scan(&owner, &original_body, &body, &timedate); err != nil {
			return systems.Post{}, err
		}
	}
	t, err := time.Parse(time.RFC3339, timedate)
	if err != nil {
		return systems.Post{}, err
	}
	id2 := systems.PostID(id)
	likes, err := GetLikeCount(id2)
	if err != nil {
		return systems.Post{}, err
	}
	comments, err := GetCommentCount(id2)
	if err != nil {
		return systems.Post{}, err
	}
	reposts, err := GetRepostCount(id2)
	if err != nil {
		return systems.Post{}, err
	}
	return systems.Post{
		ID:           id2,
		Parent:       systems.PostID(parent),
		Owner:        systems.UserID(owner),
		PostType:     post_type,
		OriginalBody: original_body,
		Body:         body,
		Likes:        likes,
		Comments:     comments,
		Reposts:      reposts,
		Time:         t,
	}, nil
}

func PostCount() (count int32, err error) {
	row := DB.QueryRow("select count(*) from posts")
	err = row.Scan(&count)
	return
}

func CreatePost(post systems.Post) (id systems.PostID, err error) {
	var res sql.Result
	if post.PostType != systems.Original {
		res, err = DB.Exec(
			"insert into posts (parent, owner, original_body, body, time, post_type) values (?, ?, ?, ?, ?, ?, ?)",
			post.Parent,
			post.Owner,
			post.Body,
			post.Body,
			post.Time.Format(time.RFC3339),
			post.PostType.String(),
		)
	} else {
		res, err = DB.Exec(
			"insert into posts (owner, original_body, body, time, post_type) values (?, ?, ?, ?, ?, ?)",
			post.Owner,
			post.Body,
			post.Body,
			post.Time.Format(time.RFC3339),
			post.PostType.String(),
		)
	}
	l, err := res.LastInsertId()
	id = systems.PostID(l)
	return
}

func GetLikeCount(id systems.PostID) (count int32, err error) {
	row := DB.QueryRow("select count(*) from likes where post_id = ?", id)
	err = row.Scan(&count)
	return
}

func LikePost(user systems.UserID, post systems.PostID) (err error) {
	_, err = DB.Exec("insert into likes (user_id, post_id) values (?, ?)", user, post)
	return
}

func UnlikePost(user systems.UserID, post systems.PostID) (err error) {
	_, err = DB.Exec("delete from likes where user_id = ? and post_id = ?", user, post)
	return
}

func QueryLike(user systems.UserID, post systems.PostID) (liked bool, err error) {
	row := DB.QueryRow("select exists(select 1 from likes where user_id = ? and post_id = ?)", user, post)
	err = row.Scan(&liked)
	return
}

func GetRepostCount(post systems.PostID) (count int32, err error) {
	row := DB.QueryRow("select count(*) from posts where parent = ? and post_type = \"repost\"", post)
	err = row.Scan(&count)
	return
}

func QueryRepost(user systems.UserID, post systems.PostID) (reposted bool, err error) {
	row := DB.QueryRow("select exists(select 1 from posts where owner = ? and parent = ? and post_type = \"repost\")", user, post)
	err = row.Scan(&reposted)
	return
}

func GetCommentCount(post systems.PostID) (count int32, err error) {
	row := DB.QueryRow("select count(*) from posts where parent = ? and post_type = \"comment\"", post)
	err = row.Scan(&count)
	return
}

func QueryComment(user systems.UserID, post systems.PostID) (commented bool, err error) {
	row := DB.QueryRow("select exists(select 1 from posts where owner = ? and parent = ? and post_type = \"comment\")", user, post)
	err = row.Scan(&commented)
	return
}

func DeletePost(id systems.PostID) (err error) {
	_, err = DB.Exec("delete from posts where id = ?", id)
	return
}

func GetPage(size int, page int, filters string) (posts []systems.Post, end bool, err error) {
	rows, err := DB.Query(
		fmt.Sprintf("select id, owner, post_type, original_body, body, time from posts where %s order by time desc limit ? offset ?", filters),
		size+1,
		page * size,
	)
	if err != nil {
		return
	}
	defer rows.Close()

	end = true
	count := 0
	for rows.Next() {
		if count == size {
			end = false
			break
		}

		var (
			id            systems.PostID
			owner         systems.UserID
			post_type     string
			original_body string
			body          string
			timedate      string
		)
		if err = rows.Scan(&id, &owner, &post_type, &original_body, &body, &timedate); err != nil {
			return
		}

		t, err := time.Parse(time.RFC3339, timedate)
		if err != nil {
			return posts, end, err
		}

		likes, err := GetLikeCount(id)
		if err != nil {
			return posts, end, err
		}
		comments, err := GetCommentCount(id)
		if err != nil {
			return posts, end, err
		}
		reposts, err := GetRepostCount(id)
		if err != nil {
			return posts, end, err
		}

		posts = append(posts, systems.Post{
			ID:           id,
			Owner:        owner,
			PostType:     systems.PostTypeFromString(post_type),
			OriginalBody: original_body,
			Body:         body,
			Likes:        likes,
			Comments:     comments,
			Reposts:      reposts,
			Time:         t,
		})

		count++
	}

	err = rows.Err()
	return
}

func EditPost(body string, id systems.PostID) (err error) {
	row := DB.QueryRow("select body from posts where id = ?", id)
	var current string
	err = row.Scan(&current)
	if err != nil {
		return
	}
	curw := strings.Split(current, " ")
	words := strings.Split(body, " ")
	changeCnt := 0
	if len(curw) != len(words) {
		err = fmt.Errorf("Incorrect word count!")
		return
	}
	for i, w := range words {
		if curw[i] != w {
			changeCnt++
		}
	}
	if changeCnt > 1 {
		err = fmt.Errorf("Too many changes!")
		return
	}
	_, err = DB.Exec("update posts set body = ? where id = ?", body, id)
	return
}
